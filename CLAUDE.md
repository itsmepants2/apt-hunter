...
| `assets/` | Static assets (icons, images) referenced by `index.html` and `manifest.json` |
| `server.py` | Local dev server: tiny Python `http.server` wrapper, port via `PORT` (default 8080); the actual story behind preview-server verification |
...
| `worker/package.json` | npm tooling for the Worker (wrangler) |
...
The `dbReady` IIFE in `archive.js` populates `_dbCache` from Supabase only when there is a session.
...
**Planned (not yet implemented):** sign-in should silently merge localStorage entries into Supabase. A one-time confirmation toast after silent migration is acceptable. Today's `onAuthStateChange` callback in `src/app.js` only re-renders the archive when a session arrives — no migration code exists yet. `mergeArchives` in `src/archive.js` is consumed by `src/sync.js` for Gist sync, not for sign-in. Do not assume a migration helper exists.

When the silent-migration flow is built, do not show a “we found 4 properties, import them?” prompt — silent merge with an optional confirmation toast.
...
There is no root `package.json`. The PWA itself is plain HTML/CSS/JS served statically.

Local dev (PWA):

```bash
python server.py        # serves repo root on PORT (default 8080)
```

Worker dev/deploy (only `worker/` has npm tooling):
...
There is no test runner, lint config, or build step at the repo root. Do not invent commands the repo doesn't support.
