...
- Core signed-in archive test passed — sign-in → save → confirm Supabase row → sign-out → no entry visible → sign back in → entry rehydrates from Supabase without hard refresh.
- Signed-out pending save test passed — save while signed out renders locally with `pendingSync: true`, survives sign-out, uploads on next sign-in.
- No duplicate Supabase rows observed across repeated sign-out/sign-in cycles.
- Account-private archive behavior is verified live.
