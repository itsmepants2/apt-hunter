...
export function clearUrlPreview() {
  ...
  urlPreviewCard.style.display = 'none';
  // reset urlPreviewData in app.js by calling clearUrlPreview — app sets it to null directly
}

export function renderUrlPreview({ url, extracted, photos }) {
  const urlPreviewCard = document.getElementById('urlPreviewCard');
  urlPreviewCard.innerHTML = '';
  urlPreviewCard.style.display = '';

  // ── Photo strip ──
  if (photos.length) {
    const strip = document.createElement('div');
    strip.className = 'url-preview-photos';
    photos.forEach(src => {
      const img = document.createElement('img');
      img.className = 'url-preview-photo';
      img.src = src;
      img.alt = '';
      img.onerror = () => { img.style.display = 'none'; };
      strip.appendChild(img);
    });
    urlPreviewCard.appendChild(strip);
  } else {
    const noPhotos = document.createElement('div');
    noPhotos.className = 'url-no-photos';
    noPhotos.textContent = 'No se encontraron fotos — puedes añadirlas manualmente';
    urlPreviewCard.appendChild(noPhotos);
  }

  // ── Source URL ──
  const srcRow = document.createElement('div');
  srcRow.className = 'url-preview-source';
  const srcLabel = document.createElement('span');
  srcLabel.className = 'url-preview-source-label';
  srcLabel.textContent = 'Fuente';
  const srcLink = document.createElement('a');
  srcLink.className = 'url-preview-link';
  srcLink.href = url;
  ...
  urlPreviewCard.appendChild(srcRow);
  ...
  fieldsGrid.className = 'url-preview-fields';
  ...
  urlPreviewCard.appendChild(fieldsGrid);

  // ── Actions ──
  const actions = document.createElement('div');
  actions.className = 'url-preview-actions';

  const saveBtn = document.createElement('button');
  saveBtn.className = 'btn btn-analyze';
  saveBtn.textContent = '💾 Guardar en archivo';
  saveBtn.addEventListener('click', () => {
    // Use the first visible photo as thumbnail; rest go to extraPhotos
    const visiblePhotos = photos.filter(src => {
      const el = urlPreviewCard.querySelector(`img[src="${CSS.escape(src)}"]`);
    ...
    const entry = {
      ...
      address:          fieldInputs.neighborhood.value || null,
      type:             null,
      price:            null,
      extras:           null,
      allPhones:        [],
      ...
      whatsappMessage:  '',
      status:           'spotted',
      notes:            fieldInputs.notes.value,
      bedrooms:         fieldInputs.bedrooms.value,
      bathrooms:        fieldInputs.bathrooms.value,
      parking:          fieldInputs.parking.value,
      priceMxn:         fieldInputs.priceMxn.value,
      sizeSqm:          fieldInputs.sizeSqm.value,
      neighborhood:     fieldInputs.neighborhood.value,
      amenities:        fieldInputs.amenities.value,
  ...
  cancelBtn.addEventListener('click', clearUrlPreview);

  actions.appendChild(saveBtn);
  actions.appendChild(cancelBtn);
  urlPreviewCard.appendChild(actions);

  urlPreviewCard.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
}
